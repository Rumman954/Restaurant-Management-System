...
export default function HomePage() {
  ...
  return (
    <>
      <section
        className="relative h-[270px] overflow-hidden bg-[#ececec] sm:h-[350px] md:h-[410px]"
      ...
      >
        ...
        <div className="absolute inset-x-0 top-[18%] flex justify-center px-4 text-center sm:top-[24%]">
          <div className="w-full max-w-2xl px-3 py-3 text-black sm:px-8 sm:py-5">
          <h1
          ...
          <p
            ...
            className="old-slider-subtitle animate-[fadeIn_0.9s_ease] font-normal leading-snug"
          >
      ...
      <section className="mx-auto max-w-6xl px-4 pb-12 pt-10 sm:pb-16 sm:pt-12">
        ...
        <div className="grid items-end gap-6 md:grid-cols-[1.7fr_1fr] md:gap-8">
          ...
          <img src="/images/cooking-6668437_1280.png" alt="Cooking illustration" className="mx-auto h-[240px] w-[260px] object-contain sm:h-[300px] sm:w-[329px] md:mr-8" />
        </div>
      </section>
        ...
        {featuredCategories.map((item) => (
          <div key={item.title} className="px-1 py-6 sm:py-10">
            <div className="group h-[350px] sm:h-[380px] [perspective:1000px]">
              ...
              >
                <article className="absolute inset-0 overflow-hidden rounded-lg bg-white p-0 shadow-sm transition duration-300 group-hover:-translate-y-1 group-hover:shadow-xl [backface-visibility:hidden]">
                  <img
                    src={item.image}
                    alt={item.title}
                    className="h-48 w-full object-cover transition duration-300 group-hover:scale-[1.04] sm:h-60"
                  />
                  <div className="p-4">
                ...
                <article className="absolute inset-0 flex h-full flex-col rounded-lg bg-white p-4 shadow-sm [backface-visibility:hidden] [transform:rotateY(180deg)]">
                  <div className="flex items-start justify-between">
                    <h3 className="text-xl font-semibold">{item.title}</h3>
                    <button
                      type="button"
                      onClick={() => setExpandedCategory(null)}
                      className="text-lg font-semibold text-zinc-700 transition hover:text-zinc-900"
                      aria-label={`Close ${item.title} details`}
                    >
                      ×
                    </button>
                  </div>
                  <p className="mt-3 text-sm leading-7 text-zinc-600">{item.details}</p>
                </article>
              </div>
            </div>
          </div>
      ...
      <div className="mb-8 mt-2 text-center sm:mt-1">
        <Link
          to="/food-categories"
          className="rounded-md bg-[#ee6e73] px-5 py-3 text-white shadow-sm transition duration-200 hover:-translate-y-0.5 hover:bg-[#e35f66] hover:shadow-md"
        >
          More Foods »
        </Link>
      </div>
      ...
      <section className="mx-auto mb-12 max-w-6xl px-4">
        <div
          className="relative h-56 overflow-hidden rounded-xl shadow-md sm:h-72 md:h-80"
      ...
      <section className="mx-auto max-w-5xl px-4 py-8 text-center">
        ...
        <Link
          to="/about"
          className="mt-6 inline-block rounded-md bg-[#ee6e73] px-5 py-2.5 text-white transition duration-200 hover:-translate-y-0.5 hover:bg-[#e35f66] hover:shadow-md"
        >
          Read More »
        </Link>
      </section>
      ...
      <section className="mx-auto grid max-w-6xl gap-5 px-4 py-10 sm:px-8 sm:py-14 md:grid-cols-3 md:gap-6">
        {[
          ...
          <div key={item.title} className="rounded-lg bg-white px-8 py-10 text-center shadow-sm sm:px-[50px] sm:py-[50px]">
      ...
      <section className="mx-auto max-w-6xl px-4 pb-10 pt-4 text-center">
        <h3 className="mb-10 text-4xl font-medium text-zinc-800 sm:mb-12 sm:text-5xl">What Our Customers Say</h3>
        <div
          ...
          {reviews.map((review, index) => {
            ...
            return (
              <article
                ...
                className="absolute left-1/2 top-1/2 h-[220px] w-[160px] rounded-sm bg-[#ee6e73] p-3 text-white shadow-md transition-all duration-700 sm:h-[245px] sm:w-[190px] sm:p-4"
                style={{
